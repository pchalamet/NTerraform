...
namespace NTerraform.Providers
{
    ...
    public sealed class consul : NTerraform.provider
    {
        ...
        [TerraformProperty(name: "address", @out: false, nested: true, min: 0, max: 1)]
        public string @Address { get; }

        [TerraformProperty(name: "ca_file", @out: false, nested: true, min: 0, max: 1)]
        public string @CaFile { get; }

        [TerraformProperty(name: "cert_file", @out: false, nested: true, min: 0, max: 1)]
        public string @CertFile { get; }

        [TerraformProperty(name: "datacenter", @out: false, nested: true, min: 0, max: 1)]
        public string @Datacenter { get; }

        [TerraformProperty(name: "http_auth", @out: false, nested: true, min: 0, max: 1)]
        public string @HttpAuth { get; }

        [TerraformProperty(name: "insecure_https", @out: false, nested: true, min: 0, max: 1)]
        public bool? @InsecureHttps { get; }

        [TerraformProperty(name: "key_file", @out: false, nested: true, min: 0, max: 1)]
        public string @KeyFile { get; }

        [TerraformProperty(name: "scheme", @out: false, nested: true, min: 0, max: 1)]
